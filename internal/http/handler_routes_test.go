...
func TestRegister_RegistersOnlyCurrentAPIRoutes(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "event securities route registered", method: http.MethodGet, path: "/api/events/e1/securities", code: http.StatusInternalServerError},
		{name: "security impacts route registered", method: http.MethodGet, path: "/api/securities/ABC/impacts", code: http.StatusInternalServerError},
